...
def process_csv(input_csv, output_json=None):
    ...
    try:
        with open(input_csv, 'r', encoding='utf-8-sig') as f:  # utf-8-sig removes BOM
            ...
            if reader.fieldnames:
                ...
                print("⚠️  No columns found in CSV!")
                return False
            ...
            for idx, row in enumerate(reader, start=1):
                ...
                # Skip rows with invalid name
                if not dish_name or dish_name == 'N/A':
                    skipped_count += 1
                    continue
                
                # Skip rows with empty ingredients (will need to fetch later)
                if not ingredients_text:
                    skipped_count += 1
                    if idx <= 10:  # Show first few skipped
                        print(f"  ⊘ Skipped [{idx}] {dish_name[:50]}... (no ingredients)")
                    continue
                
                try:
                    if idx <= 5:  # Show first few processed
